---
date created: 2025-11-26T21:32
date modified: 2025-12-09T12:27
tags:
  - resourceland
---

[What is GPU programming? \| Red Hat Developer](https://developers.redhat.com/articles/2024/08/07/what-gpu-programming) 

[Cornell Virtual Workshop: Understanding GPU Architecture](https://cvw.cac.cornell.edu/gpu-architecture)

[GPU resources \| Raph Levien’s blog](https://raphlinus.github.io/gpu/2020/02/12/gpu-resources.html) , Notably:

- [Dissecting the NVIDIA Volta GPU Architecture via Microbenchmarking](https://arxiv.org/pdf/1804.06826.pdf)
- [Dissecting the NVidia Turing T4 GPU via Microbenchmarking](https://arxiv.org/pdf/1903.07486.pdf)
- [MLIR](https://blog.tensorflow.org/2019/04/mlir-new-intermediate-representation.html)
- [Building WebGPU with Rust](https://fosdem.org/2020/schedule/event/rust_webgpu/) - FOSDEM talk
- [Get started with GPU Compute on the Web](https://developers.google.com/web/updates/2019/08/get-started-with-gpu-compute-on-the-web) - Google (Chromium/Dawn)

[GitHub - uccl-project/uccl](https://github.com/uccl-project/uccl) : UCCL is an efficient communication library for GPUs, covering collectives, P2P (e.g., KV cache transfer, RL weight transfer), and EP (e.g., GPU-driven)

[AMD GPU Debugger · Abdelhadi \| عبدالهادي](https://thegeeko.me/blog/amd-gpu-debugging/) 

[Touching the Elephant - TPUs \| Consider the Bulldog](https://considerthebulldog.com/tte-tpu/)

[My first Multi-GPU kernel: Writing All-to-all for AMD MI300X - gau-nernst's blog](https://gau-nernst.github.io/amd-a2a/) 

[notebooks/ModuleGPU/GPU\_programming\_basics.ipynb at master · dataflowr/notebooks · GitHub](https://github.com/dataflowr/notebooks/blob/master/ModuleGPU/GPU_programming_basics.ipynb) 

[Custom CPU simulation and testing](https://popovicu.com/posts/custom-cpu-simulation-and-testing/)

